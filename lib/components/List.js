...
var List = React.createClass({
  ...
  componentWillReceiveProps: function (props) {
    ...
    var keys = fill(value, this.state.keys);
    this.setState({
      hasError: false,
      value: value,
      keys: keys
  ...
  onChange: function (value, keys) {
    ...
    this.setState({hasError: false, value: value, keys: keys}, function () {
  ...
  render: function () {
    ...
    var components = this.state.value.map(function (value, i) {
      return (
        React.createElement('div', {className: 'row', key: this.state.keys[i]},
